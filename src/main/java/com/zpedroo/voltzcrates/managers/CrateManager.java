...
import com.zpedroo.voltzcrates.objects.Reward;
...
public class CrateManager {
    ...
    private Map<Block, BukkitTask> openChestTask;
    private DataCache dataCache;

    public CrateManager() {
        instance = this;
        this.openChestTask = new HashMap<>(8);
            ...
            while (!openCrate) {
                for (Reward reward : placedCrate.getCrate().getRewards()) {
                    ...
                    if (InventoryManager.getFreeSpace(player, reward.getDisplay()) <= 0) {
                        player.sendMessage(Messages.NEED_SPACE);
                        cancelled = true;
                        break amountLoop;
                    }
    ...
    public void create(Location location, Crate crate) {
        cache(new PlacedCrate(location, crate));
    ...
    public void delete(PlacedCrate placedCrate) {
        placedCrate.delete();
        dataCache.getPlacedCrates().remove(placedCrate.getLocation());
...
>>>>>>> 3b71325d45999be59b70484b7c31cb0745c4ba98
    ...
    private void cache(Crate crate) {
        dataCache.getCrates().put(crate.getName().toUpperCase(), crate);
    }

    private void cache(PlacedCrate placedCrate) {
